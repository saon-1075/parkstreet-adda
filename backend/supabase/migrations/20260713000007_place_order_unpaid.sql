-- ============================================================================
-- Payments · 0007 · Lock place_order() to UNPAID only.
-- Supersedes 0006's client-side "paid" path. Online payments are now persisted
-- ONLY by the `razorpay` Edge Function AFTER server-side signature verification.
-- place_order() (callable by anon) ignores any payment_ref and always creates an
-- UNPAID order — so nobody can self-mark an order paid by calling the RPC directly.
-- ============================================================================
...
begin
  ...
  -- Always unpaid; payment_ref is intentionally not read here.
